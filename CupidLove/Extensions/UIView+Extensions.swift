...
extension UIView {
    ...
    func insertSeparator(height: CGFloat = 1.0, color: UIColor = UIColor.lightGray, inset: CGFloat = 0.0, position: SeparatorPosition = .bottom) {
        switch position {
        case .top :
            removeSeparator()
            drawSeparator(height: height, y: 1.0, color: color, inset: inset)
        case .bottom:
            removeSeparator()
            drawSeparator(height: height, y: frame.height, color: color, inset: inset)
        case .both:
            drawSeparator(height: height, y: 1.0, color: color, inset: inset)
            drawSeparator(height: height, y: frame.height, color: color, inset: inset)
        default:
     ...
     */
    func drawSeparator(height: CGFloat = 1.0, y: CGFloat, color: UIColor, inset: CGFloat) {
    ...
    func highlight(back: UIColor = ThemeManager.shared.themeData!.pink.hexColor,
