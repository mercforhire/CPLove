//
//  MyLikedCupidsViewController.swift
//  CupidLove
//
...
class MyLikedCupidsViewController: BaseViewController {
    private let itemInfo = IndicatorInfo(title: "Liked Cupids")
    ...
    private func fetchData(complete: ((Bool) -> Void)? = nil) {
        likes.isEmpty ? FullScreenSpinner().show() : nil
        
        api.getMyLikedCupids(userId: user.identifier) { [weak self] result in
            ...
            case .success(let response):
                if let likedUsers = response.data?.records {
...
extension MyLikedCupidsViewController: IndicatorInfoProvider {
...
extension MyLikedCupidsViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
