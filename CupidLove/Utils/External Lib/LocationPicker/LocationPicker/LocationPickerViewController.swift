...
open class LocationPickerViewController: UIViewController {
    ...
    func selectLocation(location: CLLocation) {
        ...
        let completionHandler: CLGeocodeCompletionHandler = { response, error in
        ...
        if #available(iOS 11.0, *) {
            CLGeocoder().reverseGeocodeLocation(location, preferredLocale: Locale(identifier: "en_US"), completionHandler: completionHandler)
        } else {
...
extension LocationPickerViewController: UISearchResultsUpdating {
	...
	func selectedLocation(_ location: Location) {
        ...
        let completionHandler: CLGeocodeCompletionHandler = { [weak self] response, error in
            ...
            if let error = error as NSError?, error.code != 10 { // ignore cancelGeocode errors
                ...
                let location = Location(name: name, location: location.location, placemark: placemark)
        ...
        if #available(iOS 11.0, *) {
            CLGeocoder().reverseGeocodeLocation(location.location, preferredLocale: Locale(identifier: "en_US"), completionHandler: completionHandler)
        } else {
            UserDefaults.standard.set(["en_US"], forKey: "AppleLanguages")
            CLGeocoder().reverseGeocodeLocation(location.location) { placemarks, error in
                // Remove the language override
                UserDefaults.standard.removeObject(forKey: "AppleLanguages")
                completionHandler(placemarks, error)
            }
